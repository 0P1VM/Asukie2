...
module.exports = (client, message, args, prefix) => {
    ...
    let muteRole = message.guild.roles.find(r => r.name === "😡 Asukie Mute 🔇");
    if (!muteRole) {
        ...
        message.guild.createRole({ name: "😡 Asukie Mute 🔇" }).then(role => {
            ...
            message.reply("**Criei a tag: `😡 Asukie Mute 🔇` no servidor, Use o comando novamente!**");
        }).catch(() => {
            message.reply("Erro ao criar o cargo `😡 Asukie Mute 🔇`");
        });
        return 0;
    }
    if (muteMember.roles.find(r => r.name === `😡 Asukie Mute 🔇`)) {
        message.reply(`O membro ${muteMember} já está mutado`);
        return 0;
    }
    if (args.length < 2) {
        message.reply(`Exemplo de uso: \`${prefix}mute @Lu4n#0047 5m <Motivo>\``);
    }
    let time = ms(args[1]);
    if (!time || time < ms("15s")) {
    ...
    muteMember.addRole(muteRole, reason).then(() => {
        guild.findById(message.guild.id, "edit", (err, guildTable) => {
            ...
            var timeoutID = setTimeout(() => {
                if (!muteMember.roles.find(r => r.name === `😡 Asukie Mute 🔇`)) return;
